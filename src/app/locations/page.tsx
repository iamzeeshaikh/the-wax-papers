...
export const metadata: Metadata = {
  title: "UK Delivery Locations | Custom Food Papers Across the UK | The Wax Papers",
  description:
                ...
                {group.cities.map((c) => (
                  <Link key={c.slug} href={`/locations/${c.slug}`} className="rounded-xl p-5 border block transition-shadow hover:shadow-lg" style={{ backgroundColor: "var(--color-cream)", borderColor: "var(--color-paper)" }}>
                    <div className="flex items-center gap-2 mb-1">
                      <svg className="w-4 h-4 flex-shrink-0" style={{ color: "var(--color-gold)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <span className="text-sm font-semibold" style={{ color: "var(--color-gold)" }}>Food papers in {c.city} →</span>
                  </Link>
                ))}
              </div>
            </div>
