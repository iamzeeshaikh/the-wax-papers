...
const LOC_NATIONS = ["England", "Scotland", "Wales", "Northern Ireland"] as const;
const LOCATIONS_BY_NATION = LOC_NATIONS.map((nation) => ({
  nation,
      ...
      <div className="container-wide flex items-center justify-between py-3">
        ...
        <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
          ...
          <div className="relative" ref={locRef}>
            ...
            {locOpen && (
              <div
                className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-[560px] rounded-xl shadow-2xl border overflow-hidden"
                style={{ backgroundColor: "white", borderColor: "var(--color-paper)" }}
              >
                <div
                  className="px-5 py-3 text-xs font-semibold uppercase tracking-widest"
                  style={{ backgroundColor: "var(--color-cream-light)", color: "var(--color-brown)", borderBottom: "1px solid var(--color-paper)" }}
                >
                  Custom Food Papers by City — UK
                </div>
                ...
                <div
                  className="px-5 py-3 flex items-center justify-between"
                  style={{ backgroundColor: "var(--color-cream-light)", borderTop: "1px solid var(--color-paper)" }}
                >
                  <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>Nationwide UK delivery</span>
                  <Link href="/locations" onClick={() => setLocOpen(false)} className="text-sm font-semibold underline" style={{ color: "var(--color-brown)" }}>
                    View all locations →
                  </Link>
                </div>
              </div>
          ...
          <nav className="container-wide py-4 space-y-1" aria-label="Mobile navigation">
            ...
            <div className="pt-2">
              ...
              <Link
                href="/locations"
                onClick={() => setMenuOpen(false)}
                className="block px-4 py-2 text-sm font-medium text-white/85 hover:text-white hover:bg-white/10 transition-colors rounded"
              >
                All UK Locations
              </Link>
              {locations.map((c) => (
